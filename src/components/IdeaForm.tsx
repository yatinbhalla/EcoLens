...
export function IdeaForm({ onSubmit, initialIdeas }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 w-full max-w-[1024px] mx-auto text-[#1A2E22] pb-12">
      <div className="text-center space-y-4 mb-8">
        <h1 className="text-4xl font-bold tracking-tight text-[#064E3B]">Idea Analysis</h1>
        <p className="text-gray-500">Analyze the environmental impact of your product idea, or compare multiple ideas.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
        {ideas.map((idea, index) => (
          <div key={idea.id} className="p-8 rounded-[2rem] bg-white border border-[#E5E1D8] shadow-sm space-y-4 relative">
            {ideas.length > 1 && (
              <button 
                type="button" 
            ...
            <h3 className="font-bold text-xl text-[#064E3B]">Idea {index + 1}</h3>
            
            <div className="space-y-2">
              <label className="text-sm font-bold uppercase tracking-wide text-gray-500 text-[11px]">Product Name *</label>
              <Input 
                required 
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold uppercase tracking-wide text-gray-500 text-[11px]">Description *</label>
              <Textarea 
                required 
            ...
            <Accordion title="Optional Details (improves accuracy)">
              <div className="space-y-4 pt-4">
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold uppercase tracking-wide text-gray-500">Known Materials/Tech</label>
                  <Input placeholder="e.g. Bamboo, AWS servers" value={idea.materials || ''} onChange={e => handleUpdate(index, 'materials', e.target.value)} />
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-bold uppercase tracking-wide text-gray-500">Production/Hosting Location</label>
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold uppercase tracking-wide text-gray-500">Distribution / Delivery</label>
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold uppercase tracking-wide text-gray-500">Intended Lifespan</label>
        ...
        {ideas.length < 5 && (
          <button 
            type="button" 
            onClick={handleAdd}
            className="flex flex-col items-center justify-center p-8 h-full rounded-[2rem] border border-dashed border-[#E5E1D8] text-gray-400 hover:text-[#064E3B] hover:border-[#6B8E23] hover:bg-white transition-all min-h-[300px]"
          >
      ...
      <div className="flex justify-center pt-8">
        <button type="submit" disabled={ideas.every(i => !i.name || !i.description)} className="px-8 py-3 bg-[#064E3B] text-white rounded-full font-bold shadow-sm hover:bg-[#064E3B]/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
